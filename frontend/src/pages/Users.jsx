    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      {!isEdit ? (
        ...
        <div>
          ...
          <input className="input bg-gray-50 dark:bg-gray-800 text-gray-500 dark:text-gray-400" value={initial.username} disabled />
        </div>
      )}
      ...
      <div>
        ...
        {roleOpts.length === 1 ? (
          <input className="input bg-gray-50 dark:bg-gray-800 text-gray-500 dark:text-gray-400" value={roleOpts[0].label} disabled />
        ) : (
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">
          Password {isEdit
            ? <span className="text-gray-400 dark:text-gray-500 font-normal">(kosongkan jika tidak diubah)</span>
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4 md:mb-6">
        <div>
          <h2 className="text-lg md:text-xl font-semibold text-gray-800 dark:text-gray-100">{pageTitle}</h2>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[420px]">
            <thead className="bg-gray-50 dark:bg-gray-800/60 border-b border-gray-200 dark:border-gray-700">
              <tr>
                <th className="text-left px-4 md:px-6 py-3 text-gray-600 dark:text-gray-400 font-medium">Nama</th>
                <th className="text-left px-4 md:px-6 py-3 text-gray-600 dark:text-gray-400 font-medium hidden sm:table-cell">Username</th>
                <th className="text-center px-4 md:px-6 py-3 text-gray-600 dark:text-gray-400 font-medium">Role</th>
                {isSuperAdmin && (
                  <th className="text-left px-4 md:px-6 py-3 text-gray-600 dark:text-gray-400 font-medium hidden lg:table-cell">Dibuat oleh</th>
                )}
                <th className="text-center px-4 md:px-6 py-3 text-gray-600 dark:text-gray-400 font-medium">Status</th>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {users.length === 0 && (
                <tr>
                  <td colSpan={isSuperAdmin ? 6 : 5} className="text-center py-10 text-gray-400 dark:text-gray-500">
              ...
              {users.map(u => (
                <tr key={u.id} className={`hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors ${!u.is_active ? 'opacity-50' : ''}`}>
                  <td className="px-4 md:px-6 py-3">
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center text-blue-600 font-bold text-xs flex-shrink-0">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400 dark:text-gray-500 sm:hidden">{u.username}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 md:px-6 py-3 text-gray-500 dark:text-gray-400 font-mono text-xs hidden sm:table-cell">{u.username}</td>
                  <td className="px-4 md:px-6 py-3 text-center">
                    <Badge color={roleColor[u.role]}>{roleLabel[u.role]}</Badge>
                  </td>
                  {isSuperAdmin && (
                    <td className="px-4 md:px-6 py-3 text-xs text-gray-500 dark:text-gray-400 hidden lg:table-cell">
                      {u.created_by_name || <span className="italic text-gray-300 dark:text-gray-600">—</span>}
                    </td>
                  )}
                  <td className="px-4 md:px-6 py-3 text-center">
                    ...
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <button
                        className={`p-1.5 rounded-lg transition-colors ${
                          u.is_active ? 'hover:bg-red-50 dark:hover:bg-red-900/30 text-red-500' : 'hover:bg-green-50 dark:hover:bg-green-900/30 text-green-500'
